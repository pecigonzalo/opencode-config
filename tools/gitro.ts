...
// ---------------------------------------------------------------------------
// Allowlist and deny-rule definitions
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Validation helper (pure, exported for tests)
// ---------------------------------------------------------------------------
...
export function validateGitArgs(args: string[]): ValidationResult {
  ...
  // 1. Subcommand must be in the allowlist
  ...
  for (const arg of args) {
    ...
    // Prefix match for flags that can carry a value with "=" (e.g. --git-dir=...)
    ...
    // Also catch --recurse-submodules=... style (covered by prefix match above,
    // but keep explicit check as a safety net)
    ...
    // Catch -C (uppercase) as a global deny for "change dir" use
    if (arg === "-C") {
      return {
        ok: false,
  ...
  // 4. Per-subcommand rules
  const restArgs = args.slice(1);

  if (subcommand === "remote") {
    // Only allow: flags like -v / --verbose, or subverb get-url
    ...
    // Subcommands that only allow specific subverbs
    const allowedSubverbs = SUBCOMMAND_ALLOWED_SUBVERBS[subcommand]!;
    // Find the first non-flag argument (the subverb)
    ...
    for (const arg of restArgs) {
      ...
      // Handle combined short flags like -aD → check each char
...
// ---------------------------------------------------------------------------
// Error response builder (returns full JSON shape with ok:false)
// ---------------------------------------------------------------------------
